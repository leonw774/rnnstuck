...
import random
import h5py
import process_w2v_model as w2vparas
from gensim.models import word2vec
from keras import optimizers
from keras import backend as K
from keras.models import Sequential, Model, load_model
from keras.layers import Activation, Dense, dot, Dropout, Embedding, Flatten, GRU, Input, Lambda, LSTM, multiply, Permute, RepeatVector, Reshape, TimeDistributed

#os.environ["CUDA_VISIBLE_DEVICES"] = '1' # -1 : Use CPU; 0 or 1 : Use GPU

PAGE_LENGTH_MAX = 2048
PAGE_LENGTH_MIN = 4
FIXED_PAGE_LENGTH = 4 # set None to be varifyed length

USE_SAVED_MODEL = False
USE_ATTENTION = True # only relevant when FIXED_PAGE_LENGTH is given
...
PAGENAME_LIST = []
for filename in os.listdir(w2vparas.PROC_PATH) :
    PAGENAME_LIST.append(filename)
PAGENAME_LIST = sorted(PAGENAME_LIST, key = sorting_file_name)

print("W2V_BY_VOCAB: ", w2vparas.W2V_BY_VOCAB, "\niters: ", w2vparas.W2V_ITER)

### PREPARE TRAINING DATA ###
total_word_count = 0
page_list = []
for count, pagename in enumerate(PAGENAME_LIST[w2vparas.SAMPLE_BEGIN : w2vparas.SAMPLE_END]) :
    ...
    if len(this_page_word_list) < (PAGE_LENGTH_MIN if FIXED_PAGE_LENGTH == None else FIXED_PAGE_LENGTH) :
        continue    
    # put ending character at the end of a page
    if w2vparas.USE_ENDING_MARK :
        if w2vparas.W2V_BY_VOCAB :
            this_page_word_list.append(w2vparas.ENDING_MARK_WORD)
        else :
            this_page_word_list += w2vparas.ENDING_MARK_CHAR
...
if FIXED_PAGE_LENGTH :
    STEPS_PER_EPOCH /= (FIXED_PAGE_LENGTH ** 0.5)
del word_model

print("\ntotal_word_count: ", total_word_count)
print("\nvector size: ", w2vparas.WV_SIZE, "\nvocab size: ", VOCAB_SIZE)
...
def generate_sentences(batch_size = 1, sentence_length = None) :
    ...
    while 1:
        ...
        if sentence_length == None :
            ...
            x = np.zeros((batch_size, seq_length, w2vparas.WV_SIZE))
        ...
        # unified length
            ...
            for i, n in enumerate(post_nums) :
                this_length = min(x_train_list[n].shape[1], random.randint(1, seq_length))
                offset = random.randint(0, x_train_list[n].shape[1] - this_length)
                x[i, : this_length] = x_train_list[n][:, offset : this_length + offset] # index offset to seq_length + offset - 1 
                y[i, : this_length] = y_train_list[n][:, this_length + offset - 1]
        if print_counter < 0 :
...
### NETWORK MODEL ###
print("\nUSE_SAVED_MODEL:", USE_SAVED_MODEL, "\nFIXED_PAGE_LENGTH:", FIXED_PAGE_LENGTH, "\nRNN_UNIT:", RNN_UNIT, "\nbatch size:", BATCH_SIZE, "\nOUTPUT_NUMBER:", OUTPUT_NUMBER)

if USE_SAVED_MODEL :
    model = load_model(SAVE_MODEL_NAME)
else :
    sgd = optimizers.SGD(lr = 0.02, momentum = 0.5, nesterov = True, decay = (1/STEPS_PER_EPOCH))
    rmsprop = optimizers.RMSprop(lr = 0.005, decay = (1/STEPS_PER_EPOCH))
    adam = optimizers.Adam(lr = 0.001, decay = (1/STEPS_PER_EPOCH))
    
    ## make model
    input_layer = Input([FIXED_PAGE_LENGTH, w2vparas.WV_SIZE])
    if FIXED_PAGE_LENGTH :
        if USE_ATTENTION :
            rnn_layer = GRU(RNN_UNIT, return_sequences = True, stateful = False)(input_layer)
            attention = Permute((2, 1))(rnn_layer)
            attention = Dense(FIXED_PAGE_LENGTH, activation = "softmax")(attention)
            attention = Permute((2, 1))(attention)
            x = multiply([attention, rnn_layer])
            x = Flatten()(x)
        else :
           rnn_layer = GRU(RNN_UNIT, return_sequences = False, stateful = False)(input_layer)
           x = Dropout()(rnn_layer)
    else :
        rnn_layer = GRU(RNN_UNIT, return_sequences = True, stateful = False)(input_layer)
        x = Dropout()(rnn_layer)
...
model.fit_generator(generator = generate_sentences(batch_size = BATCH_SIZE, sentence_length = FIXED_PAGE_LENGTH),
                    steps_per_epoch = STEPS_PER_EPOCH, 
                    epochs = EPOCHS, 
                    verbose = 1,
                    validation_data = generate_sentences(batch_size = VALIDATION_NUMBER, sentence_length = FIXED_PAGE_LENGTH), 
                    validation_steps = 1)
...
for out_i in range(OUTPUT_NUMBER) :
    ...
    for n in range(100) :
        ...
        if FIXED_PAGE_LENGTH :
            if input_array.shape[1] > FIXED_PAGE_LENGTH :
                adjust_input_array = input_array[:, -FIXED_PAGE_LENGTH :]
            else :
                adjust_input_array = np.zeros([1, FIXED_PAGE_LENGTH, w2vparas.WV_SIZE])
